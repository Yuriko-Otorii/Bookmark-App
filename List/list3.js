...
const otherCategoriesId = [];
window.addEventListener('load', () => {
    const allCardsCategory = document.querySelectorAll('.card-category');
    allCardsCategory.forEach(e => {
        otherCategories.push(e.getAttribute('name').toLowerCase());
        otherCategoriesId.push(e.getAttribute('id'));
    });
})

function createNode(cover, id, url, title, category, categoryId){
    ...
    let cardCategoryClone = itemClone.querySelector('.card-category');
    ...
    cardCategoryClone.setAttribute('name', category);
    cardCategoryClone.setAttribute('id', categoryId);
...
const dbList = [];
const categoryArr = [];
async function getArticleAndCategory(){
    ...
    articleArr.forEach(elem => {
        const categoryIndex = dbList.findIndex(e => {
            console.log(e.category.toLowerCase());
            e.category.toLowerCase() === elem.article.category.toLowerCase()
        });
        // const categoryId = dbList[categoryIndex].id;
        // createNode(elem.article.cover, elem.id, elem.article.url, elem.article.title, elem.article.category, categoryId);
    })       
    


}
getArticleAndCategory();
...
editBtn.addEventListener('click', e => {
    ...
    const prevCategory = parentNode.querySelector(".modal-category-info").getAttribute('name').toLowerCase();
    const prevCategoryId = parentNode.querySelector(".modal-category-info").getAttribute('id');
    ...
    comfirmBtn.addEventListener('click', e => {
        ...
        httpRequests(prevCategoryId, prevCategory, newCategory, otherCategories, dblowerCaseList, dbList, edittedData, targeId);
...
//Check category diff
async function httpRequests(prevCategoryId, prevCategory, newCategory, otherCategories, dblowerCaseList, dbList, edittedData, targetArticleId, targetCategoryId){
    if(prevCategory !== newCategory){
        //Check if $prevCategory is the last one category in current cards category list   

        const prevCategoryFirstIndex = otherCategoriesId.indexOf(prevCategoryId);
        const prevCategoryLastIndex = otherCategoriesId.lastIndexOf(prevCategoryId);
        if(prevCategoryFirstIndex === prevCategoryLastIndex){
            fetch('http://localhost:3000/categories/' + prevCategoryId, 
            {
                method: 'DELETE'
            })
        }
    ...
    await fetch('http://localhost:3000/articles/' + targetArticleId, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
...
function modalContentsSet(targetNode){
    ...
    //Set category info
    const modalCategoryName = targetNode.querySelector('.card-category').getAttribute('name');
    const modalCategoryId = targetNode.querySelector('.card-category').getAttribute('id');
    modalCategoryInfo.setAttribute('name', modalCategoryName);
    modalCategoryInfo.setAttribute('id', modalCategoryId);
